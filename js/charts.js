...
function renderBodyweightChart() {
	...
	const lineColour =
		root.getPropertyValue("--md-purple-500").trim() || "#673AB7";
...
function updateStrengthChartDisplay(strengthData, exerciseName) {
	...
	if (strengthData.length === 0) {
		...
		ctx.fillText(
			"No data available for this exercise and time period",
			ctx.canvas.width / 2,
			ctx.canvas.height / 2,
		);
		return;
...
function updateTrainingLoadChart(trainingLoadData) {
	...
	for (let i = 0; i < dailyLoads.length; i++) {
		...
		const avgLoad =
			window.reduce((sum, day) => sum + day.load, 0) / window.length;
...
function updateFrequencyChart(frequencyData) {
	...
	const muscleGroups = Object.keys(muscleGroupStats)
		...
		.sort(
			(a, b) => muscleGroupStats[b].avgPerWeek - muscleGroupStats[a].avgPerWeek,
		);
	...
	if (muscleGroups.length === 0) {
		...
		ctx.textAlign = "center";
		ctx.fillText(
			"No training frequency data available",
			ctx.canvas.width / 2,
			ctx.canvas.height / 2,
		);
		return;
	...
	const frequencies = muscleGroups.map(
		(group) => muscleGroupStats[group].avgPerWeek,
	);
	const maxFrequencies = muscleGroups.map(
		(group) => muscleGroupStats[group].maxPerWeek,
	);
