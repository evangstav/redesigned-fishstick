...
function openAccessoryLogModal(itemId, liftName, setIndex) {
	...
	if (existingLog) {
		prefillWeight = BODYWEIGHT_ASSISTED_EXERCISES.includes(liftName)
			? existingLog.actualAddedWeight !== null
				? existingLog.actualAddedWeight
				: ""
			: existingLog.actualWeight !== null
				? existingLog.actualWeight
				: "";
	}
...
function submitAccessorySetLog() {
	...
	const addedWeightInput = parseFloat(
		document.getElementById("modalWeight").value,
	); // Value from modal is added weight or total weight
	...
	if (
		BODYWEIGHT_ASSISTED_EXERCISES.includes(liftName) &&
		addedWeightInput < 0
	) {
		alert(
			"Added weight for bodyweight exercises cannot be negative. Use 0 for bodyweight only.",
		);
		return;
	}
	...
	if (BODYWEIGHT_ASSISTED_EXERCISES.includes(liftName)) {
		if (userBodyweight <= 0) {
			alert(
				"Please set your bodyweight in the 'Settings & e1RMs' tab to accurately log this exercise.",
			);
			return;
		}
	...
	if (!dailyLog[itemId][`set${setIndex}`])
	...
	const planItem = USER_WORKOUT_PLAN[currentSelectedDayKey].find(
		(it) => it.id === itemId,
	);
	...
	if (CORE_LIFTS_IN_PLAN.includes(liftName)) {
		...
		const e1RMCalcFromSet = calculateEpleyE1RM(
			totalWeightLifted,
			effectiveReps,
		);
